    ...
    [FromQuery] int? company_id = null) // <--- Added Parameter
{
        ...
        private async Task<long> CountForKpiSqlAsync(string indoorColumn, bool isIndoor, string networkType, int companyId = 0)
	{
        ...
        private static string? CleanNotes(string? notes)
        {
            ...
            int idx = notes.IndexOf("@@", StringComparison.OrdinalIgnoreCase);
            if (idx >= 0)
            {
                string before = notes.Substring(0, idx);
                if (before.EndsWith("cs", StringComparison.OrdinalIgnoreCase))
                {
        ...
        private static object CleanCachedSessions(object cachedData)
        {
            ...
            try
            {
                ...
                if (doc.RootElement.ValueKind == System.Text.Json.JsonValueKind.Array)
                {
                    ...
                    foreach (var elem in doc.RootElement.EnumerateArray())
                    {
                        ...
                        {
                            if (string.Equals(prop.Name, "notes", StringComparison.OrdinalIgnoreCase))
                            {
                                dict[prop.Name] = CleanNotes(prop.Value.GetString());
                            }
                            else
                            {
                                switch (prop.Value.ValueKind)
                                {
                                    ...
                                    case System.Text.Json.JsonValueKind.Number:
                                        if (prop.Value.TryGetInt64(out long lVal)) dict[prop.Name] = lVal;
                                        else if (prop.Value.TryGetDouble(out double dVal)) dict[prop.Name] = dVal;
                                        break;
            ...
            {
                // Fallback to original cachedData if parsing fails
            }
            return cachedData;
        }
            ...
            [FromQuery] int? company_id = null)
        {
            ...
            try
            {
                ...
                if (_redis != null && _redis.IsConnected)
                {
                    ...
                    if (cached != null) return Json(CleanCachedSessions(cached));
                }
                ...
                var sessionsData = rawSessionsData.Select(s => new
                {
                    s.id,
                    s.session_name,
                    s.start_time,
