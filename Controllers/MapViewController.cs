                ...
                if (projectId.HasValue && projectId.Value > 0)
                {
                    sql += @"
                      AND EXISTS (
                          SELECT 1
                          FROM map_regions p
                          WHERE p.tbl_project_id = @projectId
                            AND p.status = 1
                            AND p.region IS NOT NULL
                            AND NOT ST_IsEmpty(p.region)
                            AND ss.start_lat IS NOT NULL
                            AND ss.start_lon IS NOT NULL
                            AND ss.end_lat IS NOT NULL
                            AND ss.end_lon IS NOT NULL
                            AND ST_Contains(p.region, ST_SRID(POINT(ss.start_lon, ss.start_lat), 4326))
                            AND ST_Contains(p.region, ST_SRID(POINT(ss.end_lon, ss.end_lat), 4326))
                      )";
                }
            ...
            cmd.CommandText = requestedVersion == "combined"
                ...
                WITH optimized_rows AS (
                    ...
                    SELECT
                        ...
                        CONVERT(COALESCE(spo.band, sp.band) USING utf8mb4) COLLATE utf8mb4_unicode_ci AS band,
                ...
                baseline_rows AS (
                    SELECT
                        ...
                        CONVERT(sp.band USING utf8mb4) COLLATE utf8mb4_unicode_ci AS band,
    ...
    [FromQuery(Name = "polygon_ids")] string? polygonIdsCsv = null)
{
    ...
    try
    {
        ...
        string BuildWhereClause(string alias, HashSet<string> columns, string? sectorColumn, string? bandColumn)
        {
            ...
            // site_id is a distinct identifier from node_b_id and is looked up independently
            // (OR'd in) so a row with a NULL node_b_id can still be found via its real site_id.
            if (hasSiteLookup && columns.Contains("site_id") && lookupSiteIds.Count > 0)
            {
                if (hasCellLookup && columns.Contains("cell_id") && lookupCellIds.Count > 0)
                    lookupClauses.Add($"({InLookup(alias, "site_id", "site_id", lookupSiteIds)} AND {InLookup(alias, "cell_id", "cell_id", lookupCellIds)})");
                else if (!hasCellLookup)
                    lookupClauses.Add(InLookup(alias, "site_id", "site_id", lookupSiteIds));
            }
