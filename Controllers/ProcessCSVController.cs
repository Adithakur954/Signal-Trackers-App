        ...
        public class SitePredictionCsvModel
{
    ...
    public string? Technology { get; set; }
        ...
        private static CsvConfiguration CreateLenientCsvConfiguration()
        {
                    ...
                    else
                    {
                        ...
                        foreach (string file in files)
                        {
                            if (fileType == 1)
                            {
                                    ...
                                    var uploadCsvName = isZipFile
                                        ? Path.GetFileName(file)
                                        : Path.GetFileName(originalFileName);
                                    if (IsColorSettingsCsv(uploadCsvName))
                                    {
                                        continue;
                                    }
    ...
    Microsoft.EntityFrameworkCore.Storage.IDbContextTransaction? outerTx = null)
{
        ...
        // Some handset exports contain malformed quoted fields. Keep processing
        // the row data CsvHelper can read instead of failing the whole upload.
    ...
    return isColValValid;
        ...
        private static bool IsColorSettingsCsv(string? fileName)
        {
            ...
            return name.StartsWith("ColorSetting_", StringComparison.OrdinalIgnoreCase) ||
